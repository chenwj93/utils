...
func ConcatFailed(data interface{}, msg ...string) (r *Response, err error) {
	...
	case 2:
		r, err = ConcatErr(TEE(msg[0] != EMPTY_STRING, msg[0], OPERATE_FAILED), http.StatusInternalServerError, http.StatusInternalServerError, nil)
	case 3:
		r, err = ConcatErr(TEE(msg[0] != EMPTY_STRING, msg[0], OPERATE_FAILED), http.StatusInternalServerError, http.StatusInternalServerError, data)
	}
	return
}
...
func ConcatErr(msg interface{}, status int, code int, data interface{}) (r *Response, err error) {
	...
	r.Json = result
	r.Code = code
	//ULog.Println(r.Code, SubString(string(r.Json),0, 500, ""))
	return r, err
}
